...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("Sidekick.audit.dir");
        System.setProperty("Sidekick.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("Sidekick.audit.dir");
            } else {
                System.setProperty("Sidekick.audit.dir", previous);
            }
        }
    }
