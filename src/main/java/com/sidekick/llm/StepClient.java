...
public class StepClient extends AbstractOpenAiCompatibleClient {
    ...
    public boolean supportsImageInput() {
        // StepFun vision models: step-1v, step-1.5v, step-1o-vision 等
        return model != null && (model.contains("-v") || model.contains("vision"));
    }

    @Override
    protected void customizeRequestBody(ObjectNode requestBody) {
        // StepFun 默认 reasoning_format=general，会返回 `reasoning` 字段；
        // deepseek-style 让它返回 Sidekick/DeepSeek 兼容的 `reasoning_content`。
