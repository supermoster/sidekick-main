...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("Sidekick.snapshot.enabled", "Sidekick_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("Sidekick.snapshot.dir", "Sidekick_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".sidekick", "snapshots").toString()));
        int max = readInt("Sidekick.snapshot.max", "Sidekick_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("Sidekick.snapshot.excludes", "Sidekick_SNAPSHOT_EXCLUDES", ""));
