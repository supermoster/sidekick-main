...
import com.sidekick.llm.LlmTraceLogger;
import com.sidekick.context.ContextProfile;
import com.sidekick.context.TokenUsageFormatter;
import com.sidekick.lsp.LspDiagnosticReport;
import com.sidekick.memory.ConversationHistoryCompactor;
import com.sidekick.memory.ExplicitMemoryHints;
import com.sidekick.memory.MemoryManager;
import com.sidekick.prompt.PromptAssembler;
import com.sidekick.prompt.PromptContext;
import com.sidekick.prompt.PromptMode;
import com.sidekick.prompt.ProjectMemoryLoader;
import com.sidekick.render.PlainRenderer;
...
import com.sidekick.image.ImageReferenceParser;
...
public class Agent {
    ...
    private String buildProjectMemoryContext() {
        try {
            ...
            log.warn("Failed to load SideKick.md project memory", e);
            return "";
        }
    }
    ...
    public String getContextStatus() {
        com.sidekick.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.sidekick.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.sidekick.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.sidekick.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.sidekick.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.sidekick.memory.MemoryEntry.estimateTokens(
