 ...
 * 1. 系统属性：{@code Sidekick.react.token.budget} / {@code Sidekick.react.stagnation.window} /
 *    {@code Sidekick.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -DSidekick.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -DSidekick.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("Sidekick.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("Sidekick.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("Sidekick.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
