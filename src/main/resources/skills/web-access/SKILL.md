...
**重要：只要工具列表里有 `mcp__chrome-devtools__*` 工具，浏览器 MCP 就是可用的——不要自行判断 Chrome 是否安装、不要输出"Chrome 未安装"之类结论，直接调用工具，调用失败再按结果判断。**
...
- `close_page` 只能关 Sidekick 自己 `new_page` 出来的 tab，不要尝试关用户原有的 Gmail / Slack。
...
```bash
ls ~/.sidekick/skills-cache/web-access/references/site-patterns/
```
...
**写回机制**：操作中如果发现新站点的陷阱或有效模式，主动写到 `~/.sidekick/skills/web-access/references/site-patterns/<domain>.md`（用户级目录，不要写 jar 内置缓存），格式参考已有文件。
...
- 不要替用户输用户名密码：Sidekick 不做自动登录
- 不要把 references/ 写回 jar 缓存目录：写到 `~/.sidekick/skills/web-access/references/site-patterns/` 用户级目录
